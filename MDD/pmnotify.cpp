...
#if (_WINCEOSVER<=700)
#else
    // user idle event processing
    if( (ppb->Message == PBT_TRANSITION) && (ppb->Flags == POWER_STATE_USERIDLE) )
    {
        KLibSetDeviceStateToIdle();
    }
#endif    PMLOCK();
...
{
    HANDLE hOwner = (HANDLE) GetCallerVMProcessId();
